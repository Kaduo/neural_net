use std::io;
use std::io::prelude::*;
use std::fs::File;

extern crate nn;
use nn::Network;

extern crate nalgebra;
use nalgebra as na;
use na::DVector;

const NB_TRAIN_SAMPLES: usize = 60_000;
const SKIP_BYTES_TRAIN_IMAGES: usize = 16;
const SKIP_BYTES_TRAIN_LABELS: usize = 8;
const IMAGE_SIZE: usize = 784;
const LABEL_SIZE: usize = 1;


fn open_mnist_train() -> Vec<(DVector<u8>, DVector<u8>)> {

	let mut res: Vec<(DVector<u8>, DVector<u8>)> = Vec::new();

	let mut train_images = File::open("resources/train-images.idx3-ubyte").unwrap();
	let mut train_labels = File::open("resources/train-labels.idx1-ubyte").unwrap();

	train_images.read(&mut [0; SKIP_BYTES_TRAIN_IMAGES]);
	train_labels.read(&mut [0; SKIP_BYTES_TRAIN_LABELS]);

	let mut image = [0; IMAGE_SIZE];
	let mut label = [0; LABEL_SIZE];
	for i in 0..NB_TRAIN_SAMPLES {
		train_images.read(&mut image);
		train_labels.read(&mut label);
		res.push((DVector::from_row_slice(IMAGE_SIZE, &image[..]),
					DVector::from_row_slice(LABEL_SIZE, &label[..])));

		println!("{:?}", i);
	}

	return res;
}

fn main() {
    open_mnist_train();
}
